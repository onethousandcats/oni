...
class OniRepo {
    ...
    delete = () => {
        ...
        console.log(`There is no repository here to remove`);
    }
...
const readObject = (repo, sha) => {
    ...
    if (_path) {
        ...
        const oniClass = ObjectFactory(format);
...
const getExistingRepo = (pathName) => {
    if (exists(path.join(pathName, ".oni"))) {
        return new OniRepo(pathName);
    }
    ...
    if (pathArray.length > 1) {
        ...
        if (exists(nextPath)) {
            return getExistingRepo(nextPath);
        }
    }
}
